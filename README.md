...
發佈版本為 **2.4.20**；外層只負責舊 `deadrecall:*` 物品 ID 遷移與十一個
...
4. 將 `deadrecall-2.4.20-bundled.jar` 放入 client 與 server 的 `mods/`。
5. 不要再放入下列十一個 Totem standalone JAR，否則 Fabric 會偵測到重複 mod ID。

## DeadRecall 2.4.20 exact module graph

| 模組 | 版本 | 功能 |
| --- | ---: | --- |
| TotemCore | `0.6.1` | 共用 API、Totem Manual、事件、migration、版本握手 |
| TotemRemnant | `0.2.12` | 背包、死亡回收、容器安全 |
| TotemDiscordBridge | `0.1.7` | Minecraft ↔ Discord 事件與稽核 |
| TotemAutomata | `0.1.13` | 銅魔像分類、採集、工具與燃料 |
| TotemAlchemy | `0.1.24` | 動態煉金、釀造與素材發現 |
| TotemEnchanting | `0.1.7` | 雕紋書櫃內容式附魔力 |
| TotemExcavation | `0.1.3` | 區域挖掘與錘具 |
| TotemLocksmith | `0.1.1` | 相連容器鎖、權限與鑰匙 |
| TotemVanillaTweaks | `0.1.9` | 原版友善調整與容器整理 |
| TotemNexus | `0.2.7` | Space Unit、好友與安全傳送 |
| TotemVillagers | `0.1.31` | 村民工作、庫存、經濟與聚落 |
...
- Automata：使用銅扳手管理銅魔像分類、採集、工具、燃料與來源／目的地。
- Nexus：註冊與探索 Space Unit、好友、地圖、成本與 server-authoritative 傳送。
- Villagers：村民擁有實際庫存、食物、工具、交易存量、工作循環與聚落生產。
- Alchemy：多結果素材釀造、成功率、Firefly Strength 等藥水與動態發現手冊。
- Enchanting：有效路徑上的雕紋書櫃依實際書本內容提供最高 64 點附魔力。
- Excavation：七級錘具、server-owned 區域選擇、分 tick 挖掘與原版掉落／保護事件。
- Locksmith：固定容器網路鎖、成員權限、實體鑰匙、automation 規則與破壞稽核。
...
輸出為 `build/libs/deadrecall-2.4.20-bundled.jar`。`bundleJar` 會重新讀取每個 nested JAR 的
