...
4. 將 DeadRecall 2.4.19 的單一發佈 JAR 放入 `mods/`。
5. Client 與 Server 使用相同版本；啟動後確認 log 同時載入十一個
   `totem-*` 模組。

> DeadRecall 2.4.19 已內含 TotemCore、Remnant、Automata、Nexus、
...
| TotemLocksmith `0.1.0` | 相連容器鎖與權限 | [TotemLocksmith](https://github.com/Yunitrish006006/TotemLocksmith) |
| TotemVanillaTweaks `0.1.8` | 整理、配方與原版調整 | [TotemVanillaTweaks](https://github.com/Yunitrish006006/TotemVanillaTweaks) |
| TotemVillagers `0.1.30` | 村民工作、庫存與聚落 | [TotemVillagers](https://github.com/Yunitrish006006/TotemVillagers) |

每個功能模組都要求 `totem-core =0.6.0` 與 Fabric API。不要混用其他
候選版本；目前 bundle 的精確版本組合記錄於
[2.4.19 Release Notes](docs/releases/2.4.19.md)。
...
輸出為 `build/libs/deadrecall-2.4.19-bundled.jar`。輸入目錄必須是同一份
release graph 驗證過的十一個 JAR；建置會從每個 JAR 的
