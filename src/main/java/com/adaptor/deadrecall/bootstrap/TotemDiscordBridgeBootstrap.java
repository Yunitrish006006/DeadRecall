...
public final class TotemDiscordBridgeBootstrap {
    ...
    public static void registerRuntime() {
        ServerMessageEvents.CHAT_MESSAGE.register((message, sender, params) ->
                DiscordBridge.sendChatMessage(sender.getName().getString(), message.decoratedContent().getString()));
    ...
    private static int updateConfig(com.mojang.brigadier.context.CommandContext<CommandSourceStack> context) {
        try {
            DiscordBridge.updateConfig(BoolArgumentType.getBool(context, "enabled"), StringArgumentType.getString(context, "workerUrl"), StringArgumentType.getString(context, "apiKey"));
    ...
    private static int addChannel(com.mojang.brigadier.context.CommandContext<CommandSourceStack> context) {
        try {
            ...
            DiscordBridge.addChannel(StringArgumentType.getString(context, "channelId"), name);
    ...
    private static int removeChannel(com.mojang.brigadier.context.CommandContext<CommandSourceStack> context) {
        ...
        try { DiscordBridge.removeChannel(id); context.getSource().sendSuccess(() -> Component.translatable("message.deadrecall.discord_config.channel_removed", id).withStyle(ChatFormatting.GREEN), true); return 1;
    ...
    private static void sendServerStatus(MinecraftServer server, String status, double tps, boolean immediate) {
        ...
        if (immediate) DiscordBridge.sendServerStatusImmediately(status, server.getPlayerList().getPlayerCount(), server.getPlayerList().getMaxPlayers(), server.getServerVersion(), tps);
        else DiscordBridge.sendServerStatus(status, server.getPlayerList().getPlayerCount(), server.getPlayerList().getMaxPlayers(), server.getServerVersion(), tps);
    }
