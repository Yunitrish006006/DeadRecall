...
- 舊 nearby-drop 掃描器、UUID 差集與雙重 server task 已從實際死亡執行路徑停用；失敗時只回到原版世界掉落。
...
- 死亡背包 pre-drop 收尾：從 `Deadrecall` 刪除已停用的舊掃描死碼，並確認游標／2×2 合成格／第三方飾品槽、環境死亡與重啟流程。
...
1. 刪除死亡背包已停用的 legacy nearby-drop 死碼，並補游標／合成格／第三方欄位與環境死亡回歸。
...
- 死亡背包 pre-drop 擷取：刪除 `Deadrecall` 內已不可達的 legacy nearby-drop collector、雙重 server task、UUID 差集與相關 Map／Set。
- 游標 ItemStack、玩家 2×2 crafting slots、外部容器與第三方飾品槽的死亡整合。
- 岩漿、仙人掌、虛空與爆炸死亡回歸，以及 Server restart、斷線、重生與死亡節點回收測試。
