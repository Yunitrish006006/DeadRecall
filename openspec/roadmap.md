...
- 傳送介面 Phase A–C 已完成：四種物品入口、Server context／session、普通羅盤能力邊界、回生羅盤自己的死亡節點偏差 -50%、書本固定磁石準備時間 -20%／石碑磨損 -25%，以及已繪製地圖的 Server coverage、食物／偏差 -20%、動態玩家重驗證與位置隱私均已完成；27 個 required GameTests 全數通過。
...
- 傳送介面 Phase D：base／final 報價明細、完整 Payload／GUI 與發佈回歸。
- 混凝土粉末掉落物硬化：Server GameTest 與 512 個 ItemEntity 壓力回歸已完成；只剩兩名以上真人玩家水流驗收。

## 待排程

- Phase A–C 已完成；Phase D 依既有 proposal、design、tasks 與 delta spec 繼續實作。
...
- 傳送介面物品特化：Phase A–C 四物品入口、Server context、回生羅盤、路線典籍與地圖覆蓋已完成；base／final 報價明細尚待完成。
