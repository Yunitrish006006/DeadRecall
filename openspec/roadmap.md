...
- 傳送介面 Phase A 已完成：普通羅盤、回生羅盤、書本與具有 map ID 的已繪製地圖可由玩家或已探索磁石來源開啟 UI；Server context／session 綁定介面類型、確切使用手與 map ID，普通羅盤仍獨占註冊、探索、好友與磁石管理能力，22 個 required GameTests 全數通過。
...
- 傳送介面 Phase B：回生羅盤死亡節點偏差特化、書本固定磁石路線特化與第一階段 UI 標示。
...
- Phase A 已完成；Phase B–D 依既有 proposal、design、tasks 與 delta spec 繼續實作。
...
3. 傳送介面 Phase B：下一步實作回生羅盤死亡節點偏差特化、書本固定磁石路線特化與第一階段 UI。
4. 傳送介面 Phase C：實作已繪製地圖覆蓋範圍、食物成本／偏差特化、動態玩家目標與隱私驗證。
5. 傳送介面 Phase D：補齊 base／final 報價明細、完整 Payload、Dedicated Server 與多人回歸。
...
- 傳送介面物品特化：Phase A 四物品入口與 Server context 已完成；回生羅盤、路線典籍、地圖覆蓋與報價明細尚待完成。
