...
- 死亡背包清空回收後依 binding 停用正確死亡節點並從 Space Unit 地圖隱藏；非 owner 回收與通知故障隔離已驗證。
...
- 死亡背包 pre-drop 收尾：確認第三方飾品槽與 addon inventory API，以及 Dedicated Server 實際重啟、同 UUID 重連／重生與世界檔 reload。
...
1. 補死亡背包第三方 inventory API 與 Dedicated Server 重啟／同 UUID 重連／重生回歸。
...
- Dedicated Server 實際重啟、同 UUID 重連／重生與世界檔 reload 測試。
